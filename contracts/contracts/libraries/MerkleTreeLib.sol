...
library MerkleTreeLib {
    ...
    function _hash(bytes32 left, bytes32 right) internal pure returns (bytes32) {
        return bytes32(Field.Type.unwrap(Poseidon2.hash_2(uint256(left).toField(), uint256(right).toField())));
    }
    ...
    function initialize(Tree storage self, uint32 _depth) internal {
        ...
        self.depth = _depth;
        self.tree = new bytes32[][](_depth);
        self.currentRoot = bytes32(0);
    }

    function insert(Tree storage self, bytes32 leaf) internal returns (uint32 leafIndex) {
        ...
        for (uint32 level = 0; level < self.depth; level++) {
            ...
            if (siblingNode == bytes32(0)) {
                ...
                bytes32 leftInput = isRightChild ? siblingNode : currentComputedNode;
                bytes32 rightInput = isRightChild ? currentComputedNode : siblingNode;
    ...
    function getSiblings(Tree storage self, uint32 leafIndex) internal view returns (bytes32[] memory siblings) {
        require(leafIndex < self.tree[0].length && leafIndex >= 0, "MerkleTree: leafIndex out of bounds");
