...
contract ClearingHouse is IClearingHouse, Ownable {
    ...
    constructor(
        ...
        require(_collateralTokenAddress != address(0), "CH: Invalid collateral token address");
        require(_positionLedgerAddress != address(0), "CH: Invalid position ledger address");
    ...
    // --- Admin Functions ---
    function setPositionLedger(address newPositionLedger) external override onlyOwner {
        require(newPositionLedger != address(0), "CH: Invalid position ledger address");
        positionLedger = IPositionLedger(newPositionLedger);
    }

    function setCollateralToken(address _newCollateralToken, bool isSupported /* ignored */) external override onlyOwner {
        require(_newCollateralToken != address(0), "CH: Invalid collateral token address");
    ...
    function setFees(uint256 _takerFee, uint256 _makerFee, uint256 _liquidationFee) external override onlyOwner {
    ...
    function setPriceOracle(address baseAsset, address newOracle) external override onlyOwner {
        require(supportedBaseAssets[baseAsset], "CH: Base asset not supported");
        require(newOracle != address(0), "CH: Invalid oracle address");
        priceOracles[baseAsset] = IPriceOracle(newOracle);
    }

    function setVirtualAmm(address baseAsset, address newAmm) external override onlyOwner {
        require(supportedBaseAssets[baseAsset], "CH: Base asset not supported");
    ...
    function depositCollateral(address, /*collateralTokenAddress - ignored, uses contract's*/ uint256 amount)
        external override
    {
    ...
    function withdrawCollateral(address, /*collateralTokenAddress - ignored*/ uint256 amount)
        external override
    {
        if (amount == 0) revert InvalidAmount();
        // require(collateralTokenAddress == address(collateralToken), "CH: Unsupported collateral token");
    ...
    function getAccountBalance(address trader, address /*_collateralToken*/) external view override returns (uint256) {
    ...
    function openPosition(
        ...
        if (leverage == 0 || leverage > (100 * LEVERAGE_PRECISION)) revert InvalidLeverage(); // Max 100x
        ...
        if (traderBalances[msg.sender] < marginAmount) revert InsufficientFreeCollateral();

        // --- Calculations ---
        // Position size in quote asset terms (e.g., USDC terms)
        // positionSizeQuote = marginAmount * (leverage / LEVERAGE_PRECISION)
        uint256 positionSizeQuote = marginAmount.mul(leverage, LEVERAGE_PRECISION);
        ...
        if (collateralDecimals < 18) {
            positionSizeQuoteScaledForAmm = positionSizeQuote * (10**(18 - collateralDecimals));
        } else if (collateralDecimals > 18) {
            positionSizeQuoteScaledForAmm = positionSizeQuote / (10**(collateralDecimals - 18));
        } else {
        ...
        if (side == IPositionLedger.PositionSide.LONG) {
            ...
            (actualBaseAmount, newAmmBaseReserve, newAmmQuoteReserve) = amm.swapQuoteForBasePreview(positionSizeQuoteScaledForAmm);
        } else { // SHORT
            ...
            (actualBaseAmount, newAmmBaseReserve, newAmmQuoteReserve) = amm.swapBaseForQuotePreview(positionSizeQuoteScaledForAmm);
            ...
            // dx = (dy * x) / (y - dy)
            (uint256 currentAmmBase, uint256 currentAmmQuote) = amm.getReserves();
            uint256 numerator = positionSizeQuoteScaledForAmm * currentAmmBase;
            uint256 denominator = currentAmmQuote - positionSizeQuoteScaledForAmm; // this is dy
            if (currentAmmQuote <= positionSizeQuoteScaledForAmm) revert InsufficientLiquidity(); // Cannot take out more quote than available
            ...
            newAmmQuoteReserve = currentAmmQuote - positionSizeQuoteScaledForAmm;
        ...
        uint256 entryPrice = positionSizeQuoteScaledForAmm.mul(PRICE_PRECISION, actualBaseAmount); // quote/base, scaled

        // --- Calculate Fee ---
        uint256 feeAmountQuoteScaled = positionSizeQuoteScaledForAmm.mul(takerFeeBps, FEE_PRECISION);
        // Convert fee back to collateral's native decimals
        uint256 feeAmountCollateralDecimals;
        if (collateralDecimals < 18) {
            feeAmountCollateralDecimals = feeAmountQuoteScaled / (10**(18 - collateralDecimals));
        } else if (collateralDecimals > 18) {
            feeAmountCollateralDecimals = feeAmountQuoteScaled * (10**(collateralDecimals - 18));
        } else {
            feeAmountCollateralDecimals = feeAmountQuoteScaled;
        }


        if (traderBalances[msg.sender] < marginAmount + feeAmountCollateralDecimals) {
            revert InsufficientFreeCollateral(); // Not enough for margin + fee
        }

        // --- State Updates ---
        // 1. Deduct margin and fee from trader's free balance
        traderBalances[msg.sender] = traderBalances[msg.sender].sub(marginAmount).sub(feeAmountCollateralDecimals);
        ...
        IPositionLedger.Position memory existingPosition = positionLedger.getPosition(msg.sender, baseAsset);
        if (existingPosition.size == 0) {
            ...
            if (existingPosition.side != side) {
                ...
                revert("CH: Position with opposite side exists. Close it first.");
            }
    ...
    function closePosition(
        ...
        IPositionLedger.Position memory pos = positionLedger.getPosition(msg.sender, baseAsset);
        ...
        if (pos.side == IPositionLedger.PositionSide.LONG) {
            ...
            (quoteValueFromAmm,  ,  ) = amm.swapBaseForQuotePreview(actualBaseAmountToTrade);
        } else { // SHORT
            // Trader buys base back with quote
            (quoteValueFromAmm, , ) = amm.swapQuoteForBasePreview(actualBaseAmountToTrade);
            ...
            (uint256 currentAmmBase, uint256 currentAmmQuote) = amm.getReserves();
            if (currentAmmBase <= actualBaseAmountToTrade) revert InsufficientLiquidity(); // Cannot buy more base than available
        ...
        uint256 entryValueQuoteScaled = pos.size.mul(pos.entryPrice, PRICE_PRECISION);
        ...
        if (pos.side == IPositionLedger.PositionSide.LONG) {
            pnlScaled = int256(quoteValueFromAmm) - int256(entryValueQuoteScaled);
        } else { // SHORT
            pnlScaled = int256(entryValueQuoteScaled) - int256(quoteValueFromAmm); // Cost to buy back
        }

        // --- Calculate Fee ---
        // Fee is on the total position size being closed (quoteValueFromAmm)
        uint256 feeAmountQuoteScaled = quoteValueFromAmm.mul(takerFeeBps, FEE_PRECISION);
        ...
        if (collateralDecimals < 18) {
            pnlCollateralDecimals = pnlScaled / int256(10**(18 - collateralDecimals));
            feeCollateralDecimals = feeAmountQuoteScaled / (10**(18 - collateralDecimals));
        } else if (collateralDecimals > 18) {
            pnlCollateralDecimals = pnlScaled * int256(10**(collateralDecimals - 18));
            feeCollateralDecimals = feeAmountQuoteScaled * (10**(collateralDecimals - 18));
        } else {
        ...
        if (pnlCollateralDecimals >= 0) {
            quoteReturnedBeforeFee = pos.margin + uint256(pnlCollateralDecimals);
        } else {
            ...
            if (uint256(-pnlCollateralDecimals) > pos.margin) { // Loss exceeds margin
                quoteReturnedBeforeFee = 0; // Margin wiped out
            } else {
                quoteReturnedBeforeFee = pos.margin - uint256(-pnlCollateralDecimals);
            }
        }
        if (quoteReturnedBeforeFee < minQuoteAmountOut) revert SlippageExceeded();
        ...
        if (pnlCollateralDecimals >= 0) { // Profit
            finalAmountToTrader = pos.margin + uint256(pnlCollateralDecimals) - feeCollateralDecimals;
            traderBalances[msg.sender] = traderBalances[msg.sender].add(finalAmountToTrader);
        } else { // Loss
            ...
            if (lossAmount + feeCollateralDecimals >= pos.margin) {
                ...
                finalAmountToTrader = pos.margin - lossAmount - feeCollateralDecimals;
                traderBalances[msg.sender] = traderBalances[msg.sender].add(finalAmountToTrader);
            }
        }
        // TODO: Send fee to treasury/insurance fund

        emit PositionClosedCH(msg.sender, baseAsset, uint256(pnlCollateralDecimals), feeCollateralDecimals);
    }


    function addMargin(address baseAsset, uint256 quoteAmount) external override {
        if (!supportedBaseAssets[baseAsset]) revert AssetNotSupported();
        if (quoteAmount == 0) revert InvalidAmount();

        IPositionLedger.Position memory pos = positionLedger.getPosition(msg.sender, baseAsset);
        if (pos.size == 0) revert("CH: No position to add margin to");
        if (traderBalances[msg.sender] < quoteAmount) revert InsufficientFreeCollateral();

        traderBalances[msg.sender] = traderBalances[msg.sender].sub(quoteAmount);
        positionLedger.addMargin(msg.sender, baseAsset, quoteAmount);

        emit PositionMarginAdjusted(msg.sender, baseAsset, int256(quoteAmount), pos.margin + quoteAmount);
    }

    function removeMargin(address baseAsset, uint256 quoteAmount) external override {
        if (!supportedBaseAssets[baseAsset]) revert AssetNotSupported();
        if (quoteAmount == 0) revert InvalidAmount();

        IPositionLedger.Position memory pos = positionLedger.getPosition(msg.sender, baseAsset);
        if (pos.size == 0) revert("CH: No position to remove margin from");
        if (quoteAmount > pos.margin) revert("CH: Cannot remove more margin than available in position");
        ...
        bool success = positionLedger.removeMargin(msg.sender, baseAsset, quoteAmount);
        require(success, "CH: PositionLedger failed to remove margin"); // Should not fail if checks pass

        traderBalances[msg.sender] = traderBalances[msg.sender].add(quoteAmount);
    ...
    function liquidate(address trader, address baseAsset) external override {
        ...
        IPositionLedger.Position memory pos = positionLedger.getPosition(trader, baseAsset);
        ...
        if (pos.side == IPositionLedger.PositionSide.LONG) {
            ...
            (uint256 currentAmmBase, uint256 currentAmmQuote) = amm.getReserves();
            if (currentAmmBase <= liquidatedSizeBase) { /* Handle insufficient AMM liquidity for liquidation */ }
            uint256 costToBuyBackBase = (liquidatedSizeBase * currentAmmQuote) / (currentAmmBase - liquidatedSizeBase);
        ...
        int256 pnlScaled;
        if (pos.side == IPositionLedger.PositionSide.LONG) {
            pnlScaled = int256(quoteValueFromClosing) - int256(entryValueQuoteScaled);
        } else {
            pnlScaled = int256(entryValueQuoteScaled) - int256(quoteValueFromClosing); // (entry "sale" price - exit "buy" cost)
        }

        // Convert PnL to collateral decimals
        int256 pnlCollateralDecimals;
        if (collateralDecimals < 18) pnlCollateralDecimals = pnlScaled / int256(10**(18 - collateralDecimals));
        else if (collateralDecimals > 18) pnlCollateralDecimals = pnlScaled * int256(10**(collateralDecimals - 18));
        ...
        if (pos.side == IPositionLedger.PositionSide.LONG) positionValueAtLiquidationQuote = quoteValueFromClosing;
        ...
        uint256 liquidatorFeeQuoteScaled = positionValueAtLiquidationQuote.mul(liquidationFeeBps, FEE_PRECISION);
        uint256 liquidatorFeeCollateralDecimals;
        if (collateralDecimals < 18) liquidatorFeeCollateralDecimals = liquidatorFeeQuoteScaled / (10**(18 - collateralDecimals));
        else liquidatorFeeCollateralDecimals = liquidatorFeeQuoteScaled * (10**(collateralDecimals - 18));
        ...
        if (pos.margin >= liquidatorFeeCollateralDecimals) {
            ...
            traderBalances[msg.sender] = traderBalances[msg.sender].add(amountToLiquidator);
        } else {
        ...
        positionLedger.liquidatePosition(trader, baseAsset, msg.sender, liquidatedSizeBase);
        ...
        emit LiquidationInitiated(trader, baseAsset, msg.sender, liquidatedSizeBase, markPrice, amountToLiquidator);
    ...
    // --- View Functions ---
    function getMarkPrice(address baseAsset) public view override returns (uint256) {
        if (!supportedBaseAssets[baseAsset]) revert AssetNotSupported();
        IVirtualAMM amm = virtualAmms[baseAsset];
    ...
    function getIndexPrice(address baseAsset) public view override returns (uint256) {
        if (!supportedBaseAssets[baseAsset]) revert AssetNotSupported();
    ...
    function getPosition(address trader, address baseAsset) public view override returns (IPositionLedger.Position memory) {
        if (!supportedBaseAssets[baseAsset] && !positionLedger.hasPosition(trader, baseAsset) ) {
