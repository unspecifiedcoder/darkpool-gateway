...
import { ClearingHouseV2, MockERC20, Oracle } from "../../typechain-types";
...
const generateId = () => ethers.randomBytes(32); // Helper for unique position IDs

describe("ClearingHouseV2 Contract Tests", function () {
    // Actors
    ...
    let oracle: Oracle;
    let clearingHouse: ClearingHouseV2; // V2 Type

    // Constants
    const INITIAL_USDC_MINT = parseUSDC("100000");
    const INITIAL_BTC_PRICE = parsePrice("50000");
    const LEVERAGE = 10n * 100n; // 10x leverage
    ...
    beforeEach(async function () {
        ...
        const ClearingHouseV2Factory = await ethers.getContractFactory("ClearingHouseV2", deployer);
        clearingHouse = await ClearingHouseV2Factory.deploy(oracleAddress, usdcTokenAddress);
        ...
        for (const trader of [alice, bob, charlie, david]) {
    ...
    describe("V2: Multi-Position Management", function () {
        let longPositionId: string;
        let shortPositionId: string;

        beforeEach(async function() {
            // Alice deposits collateral for two positions
            await clearingHouse.connect(alice).depositCollateral(parseUSDC("2000"));
            longPositionId = ethers.hexlify(generateId());
            shortPositionId = ethers.hexlify(generateId());
        });

        it("Should allow Alice to open multiple independent positions", async function () {
            // Open a 10x long position with 100 USDC
            await clearingHouse.connect(alice).openPosition(longPositionId, parseUSDC("100"), LEVERAGE, true);
            
            // Open a 5x short position with 200 USDC
            await clearingHouse.connect(alice).openPosition(shortPositionId, parseUSDC("200"), LEVERAGE / 2n, false);
            
            const longPos = await clearingHouse.positions(longPositionId);
            const shortPos = await clearingHouse.positions(shortPositionId);

            expect(longPos.owner).to.equal(alice.address);
            expect(longPos.isLong).to.be.true;
            expect(longPos.margin).to.be.lt(parseUSDC("100")); // Margin after fee

            expect(shortPos.owner).to.equal(alice.address);
            expect(shortPos.isLong).to.be.false;
            expect(shortPos.margin).to.be.lt(parseUSDC("200"));

            // Check that free collateral was deducted for both
            expect(await clearingHouse.freeCollateral(alice.address)).to.equal(parseUSDC("1700"));
        });

        it("Should allow Alice to manage margin on one position without affecting the other", async function () {
            await clearingHouse.connect(alice).openPosition(longPositionId, parseUSDC("100"), LEVERAGE, true);
            await clearingHouse.connect(alice).openPosition(shortPositionId, parseUSDC("200"), LEVERAGE / 2n, false);
            
            const longPosBefore = await clearingHouse.positions(longPositionId);

            // Add margin to the long position
            const marginToAdd = parseUSDC("50");
            await clearingHouse.connect(alice).addMargin(longPositionId, marginToAdd);
            
            const longPosAfter = await clearingHouse.positions(longPositionId);
            const shortPosAfter = await clearingHouse.positions(shortPositionId); // Should be unchanged

            expect(longPosAfter.margin).to.equal(longPosBefore.margin + marginToAdd);
            expect(shortPosAfter.margin).to.be.lt(parseUSDC("200")); // Still has original margin
        });

        it("Should allow Alice to close one position while keeping the other open", async function () {
            await clearingHouse.connect(alice).openPosition(longPositionId, parseUSDC("100"), LEVERAGE, true);
            await clearingHouse.connect(alice).openPosition(shortPositionId, parseUSDC("200"), LEVERAGE / 2n, false);
            
            // Close the short position
            await clearingHouse.connect(alice).closePosition(shortPositionId);
            
            const longPos = await clearingHouse.positions(longPositionId);
            const shortPos = await clearingHouse.positions(shortPositionId);

            expect(longPos.owner).to.equal(alice.address); // Long position still exists
            expect(shortPos.owner).to.equal(ethers.ZeroAddress); // Short position is deleted
        });
    });

    describe("V2: Access Control", function () {
        let alicesPositionId: string;

        beforeEach(async function() {
            await clearingHouse.connect(alice).depositCollateral(parseUSDC("1000"));
            alicesPositionId = ethers.hexlify(generateId());
            await clearingHouse.connect(alice).openPosition(alicesPositionId, parseUSDC("100"), LEVERAGE, true);
        });

        it("Should REVERT if Bob tries to close Alice's position", async function () {
            await expect(clearingHouse.connect(bob).closePosition(alicesPositionId))
                .to.be.revertedWithCustomError(clearingHouse, "NotPositionOwner");
        });

        it("Should REVERT if Bob tries to add margin to Alice's position", async function () {
            await clearingHouse.connect(bob).depositCollateral(parseUSDC("100"));
            await expect(clearingHouse.connect(bob).addMargin(alicesPositionId, parseUSDC("50")))
                .to.be.revertedWithCustomError(clearingHouse, "NotPositionOwner");
        });

        it("Should REVERT if Bob tries to remove margin from Alice's position", async function () {
            await expect(clearingHouse.connect(bob).removeMargin(alicesPositionId, parseUSDC("10")))
                .to.be.revertedWithCustomError(clearingHouse, "NotPositionOwner");
        });
    });

    describe("V2: Liquidation Scenarios", function () {
        let riskyPositionId: string;

        beforeEach(async function() {
            await clearingHouse.connect(alice).depositCollateral(parseUSDC("1000"));
            riskyPositionId = ethers.hexlify(generateId());
            // Open a high-leverage long position that is vulnerable
            await clearingHouse.connect(alice).openPosition(riskyPositionId, parseUSDC("100"), 45n * 100n, true);
        });

        it("Should allow David to liquidate Alice's specific underwater position", async function () {
            // Price drops, making the position insolvent
            const newPrice = parsePrice("48000");
            await oracle.connect(deployer).setPrice(newPrice);
            
            const [, isSolvent] = await clearingHouse.calculatePnl(riskyPositionId);
            expect(isSolvent).to.be.false;

            // David (liquidator) can liquidate the specific position
            await expect(clearingHouse.connect(david).liquidate(riskyPositionId))
                .to.emit(clearingHouse, "PositionLiquidated")
                .withArgs(alice.address, riskyPositionId, david.address, (fee: bigint) => fee > 0);

            const position = await clearingHouse.positions(riskyPositionId);
            expect(position.owner).to.equal(ethers.ZeroAddress); // Position is deleted
        });
    });
    ...
    describe("Collateral Management", function () {
        ...
        describe("Long Position Scenarios", function () {
            ...
            it("Should open a long position and update state correctly", async function () {
                let positionId = ethers.hexlify(generateId());
                const margin = parseUSDC("100"); // Using 100 USDC as margin
                await clearingHouse.connect(alice).openPosition(positionId, margin, LEVERAGE, true); // true for long
            ...
            it("Should close a profitable long position and mint profits", async function () {
                ...
                await clearingHouse.connect(alice).openPosition(positionId, margin, LEVERAGE, true);
                ...
                await clearingHouse.connect(alice).closePosition(positionId);
            ...
            it("Should close a losing long position and deduct from margin", async function () {
                ...
                const [pnl, ] = await clearingHouse.calculatePnl(positionId);
         ...
         describe("Short Position Scenarios", function () {
                ...
                it("Should open a short position correctly", async function () {
                    const positionId = ethers.hexlify(generateId());
                     const margin = parseUSDC("200");
                    await clearingHouse.connect(bob).openPosition(positionId, margin, LEVERAGE, false); // false for short
                ...
                it("Should close a profitable short position", async function () {
                    const positionId = ethers.hexlify(generateId());
                    const margin = parseUSDC("200");
                    await clearingHouse.connect(bob).openPosition(positionId, margin, LEVERAGE, false);
                    ...
                    await clearingHouse.connect(bob).closePosition(positionId);
             ...
             describe("Liquidation Scenarios", function () {
                    it("Should allow David to liquidate Alice's underwater long position", async function () {
                        ...
                        const margin = parseUSDC("100");
                        await clearingHouse.connect(alice).openPosition(positionId, margin, LEVERAGE, true); // 10x long on 100 USDC margin
                        ...
                        const [, isSolvent] = await clearingHouse.calculatePnl(positionId);
                        expect(isSolvent).to.be.false;
            
                        const davidBalanceBefore = await usdcToken.balanceOf(david.address);
            
                        // David liquidates Alice
                        await expect(clearingHouse.connect(david).liquidate(positionId))
                            .to.emit(clearingHouse, "PositionLiquidated")
                            .withArgs(alice.address, positionId, david.address, (fee: bigint) => fee > 0);
                    ...
                    it("Should allow a user to add margin to avoid liquidation", async function () {
                        ...
                        const margin = parseUSDC("100");
                        await clearingHouse.connect(charlie).openPosition(positionId, margin, LEVERAGE, true);
                        ...
                        const [, isSolventBefore] = await clearingHouse.calculatePnl(positionId);
                        ...
                        await clearingHouse.connect(charlie).addMargin(positionId, additionalMargin);
                        
                        const position = await clearingHouse.positions(positionId);
                        expect(position.margin).to.equal(parseUSDC("99") + additionalMargin); // Initial margin after fee + new margin
            
                        const [, isSolventAfter] = await clearingHouse.calculatePnl(positionId);
