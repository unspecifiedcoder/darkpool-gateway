...
export const HistoryTable = () => {
  ...
  const fetchHistory = useCallback(async (cursor?: string) => {
    ...
    try {
      ...
      if (tradingMode === 'Private' && userClient) {
        ...
        response = await apiService.getPublicHistoricalPositions(address!, cursor);
      }
  ...
  const formatCurrency = (value: string) => {
    ...
    return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(parseFloat(formatted));
  };
    ...
    <div className="p-6 space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-sm">Your closed and liquidated positions will appear here.</p>
        </div>
      ) : (
        <>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {
                history.map((trade) => (
                  <tr key={trade.position_id} className="border-b border-border/50 hover:bg-primary/5">
                    ...
                    <td className="p-2 text-right font-mono">{formatCurrency(trade.entry_price)}</td>
                    <td className={`p-2 text-right font-mono ${trade.status === 'Liquidated' || BigInt(trade.final_pnl) < 0 ? 'text-destructive' : 'text-success'}`}>
                      {trade.status !== 'Liquidated' ? formatCurrency(trade.final_pnl) : '-'}
                    </td>
                    <td className="p-2 text-center">
                      <span className={trade.status === 'Liquidated' ? 'text-destructive' : 'text-muted-foreground'}>
                        {trade.status}
                      </span>
                    </td>
                  </tr>
