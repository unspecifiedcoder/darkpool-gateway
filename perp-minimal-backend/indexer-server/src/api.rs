...
async fn get_open_positions(State(db): AppState, headers: HeaderMap) -> Result<Json<Value>, StatusCode> {
    // println!("[API] Received request for GET /positions/open");
    let owner_pub_key = check_auth(&headers).await?;
    println!("[API] Attempting to get open positions for public key: {:?}", hex::encode(owner_pub_key));
    let positions = db.get_open_positions(&owner_pub_key).map_err(|e| {
        // println!("[API] Error getting open positions from database: {}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
    // println!("[API] Successfully retrieved {} open positions", positions.len());
...
async fn get_unspent_notes(State(db): AppState, headers: HeaderMap) -> Result<Json<Value>, StatusCode> {
    ...
    let receiver_hash_header = headers.get("x-receiver-hash").and_then(|h| h.to_str().ok()).ok_or_else(|| {
        // println!("[API] Error: x-receiver-hash header missing or invalid");
        StatusCode::BAD_REQUEST
    })?;
    // println!("[API] Found x-receiver-hash header {}" , receiver_hash_header);

    let receiver_hash = hex::decode(receiver_hash_header.strip_prefix("0x").unwrap_or(receiver_hash_header)).map_err(|e| {
...
async fn set_metadata(State(db): AppState, headers: HeaderMap, body: axum::body::Bytes) -> Result<StatusCode, StatusCode> {
    ...
    db.user_metadata.insert(owner_pub_key, body.to_vec()).map_err(|e| {
